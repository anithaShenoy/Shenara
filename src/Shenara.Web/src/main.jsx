...
function AdminDashboard({ route, token, onLogout }) {
  ...
  async function refreshItems() {
    request('/inventory').then(setItems).catch(() => setItems([]));
  }
    ...
    <div className="admin-shell">
      ...
      <main className="admin-main">
          ...
          <div className="metric-row">
            {activeView === 'inquiries' ? (
              ...
              <>
                ...
                <Metric label="Low Stock" value={items.filter((item) => item.availableQuantity <= item.minimumStockAlert).length} />
                <Metric label="Featured" value={items.filter((item) => item.isFeatured).length} />
              </>
            )}
          </div>
        </div>
        ...
        {activeView === 'inquiries' ? (
          ...
          <InventoryView token={token} items={items} onRefresh={refreshItems} />
        )}
      </main>
    </div>
  );
...
function InventoryView({ token, items, onRefresh }) {
  ...
  const visibleItems = useMemo(() => {
    return items.filter((item) => {
      const matchesSearch = item.name.toLowerCase().includes(query.toLowerCase()) || item.categoryName.toLowerCase().includes(query.toLowerCase());
  ...
  async function save(event) {
    ...
    const path = editingId ? `/inventory/${editingId}` : '/inventory';
    await request(path, {
  ...
  return (
        <section className="admin-grid">
          <form className="inventory-form" onSubmit={save}>
            ...
            <input required placeholder="Item name" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} />
            ...
            {editingId && <button className="quiet-button" type="button" onClick={() => { setEditingId(null); setForm(emptyInventory); }}>Cancel Edit</button>}
          ...
          <section className="inventory-panel">
            ...
            <div className="inventory-table">
              ...
              {!visibleItems.length && <div className="empty-state">No inventory items match this view.</div>}
            </div>
          </section>
        </section>
  );
